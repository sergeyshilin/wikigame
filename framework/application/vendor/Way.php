...
class Way {
    ...
    function __construct($_id, $_depth, $_links, $db_) {
        if (!is_array($_id)) {
            $this->db = $db_;
            $this->id = $_id;
            $this->setWayInfo();
    ...
    private function setWayInfo() {
        $id = $this->id;
        $info = $this->db->getAssoc("SELECT * FROM ways WHERE id = '{$id}'")[0];
        $this->hash = $info["hash"];
        $this->lang = $info["lang"];
        $this->way = Way::getWayById($this->id, $this->db);
    }

    public static function getWayById($id, $db) {
        $result = [];
        $way = $db->getAssoc("SELECT * FROM way_nodes WHERE way_id = '{$id}' order by parent_id");
